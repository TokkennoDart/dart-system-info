...
class GpuDevice extends PciDevice {
  GpuDevice.fromPci(PciDevice pci) {
    ...
    this.deviceName = pci.deviceName;
    this.vendorName = pci.vendorName;
    this.pciType = pci.pciType;
    this.pciTypeName = pci.pciTypeName;
  }
  ...
  Future<List<TempStatus>> getTemperatures() async {
    log("Temperature monitor is not supported in this system", level: 50, zone: Zone.current);
    return [];
  }

  /// Obtains an array of temperatures with the tuple current_temperature <=> max_temperature for all sensors of the GPU
  Future<List<FanStatus>> getFans() async {
    log("Fan monitor is not supported in this system", level: 50, zone: Zone.current);
    return [];
  }
